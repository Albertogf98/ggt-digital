...
export default function Services() {
  ...
  return (
    <main className="overflow-x-hidden" style={{ backgroundColor: builder.cardBg, color: builder.textColor }} id="services">
      {/* HERO */}
      <section className="relative min-h-[60vh] md:min-h-[80vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6 max-w-3xl mx-auto">
          <h1 className={`text-3xl sm:text-5xl md:text-6xl font-extrabold mb-4 leading-tight ${builder.heading}`} style={{ color: builder.heading }}>
            {t('services.title')}
          </h1>
          <p className="text-white/90 text-lg md:text-xl mb-8">{t('services.subtitle')}</p>
          <Link
            to="/contact"
            className="no-underline inline-block font-semibold px-8 py-3 rounded-full shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300"
      ...
      <section className="py-16 md:py-20">
        <div className="max-w-6xl mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map(service => (
            <div
              key={service.title}
              className="relative rounded-2xl p-8 text-center shadow-lg overflow-hidden group transition-transform transform hover:scale-105 hover:shadow-2xl"
              style={{ backgroundColor: builder.inputBg }}
            >
              <div className="absolute inset-0 bg-gradient-to-br from-blue-500/20 to-indigo-600/20 opacity-20 group-hover:opacity-40 transition-opacity rounded-2xl" />
              <div className="relative z-10 flex flex-col items-center justify-center h-full">
                ...
                <p style={{ color: builder.textColor }}>{service.text}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="py-16 px-4 text-center">
        <h2 className="text-3xl md:text-4xl font-bold mb-10" style={{ color: builder.heading }}>
          {t('services.beneficts.title')}
        </h2>
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div
              key={item.title}
              className="p-6 rounded-xl shadow-lg hover:scale-105 transition-all duration-300"
            ...
            >
              ...
              <h3 className="text-xl font-semibold mb-2" style={{ color: builder.heading }}>
                {item.title}
              </h3>
              <p>{item.text}</p>
            </div>
      ...
      <section className="relative flex flex-col md:flex-row items-center justify-center overflow-hidden py-16 px-6 lg:px-20" style={{ backgroundColor: builder.cardBg }}>
        <div className="w-full max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12">
          ...
          <div className="flex-1 text-center md:text-left max-w-xl">
            ...
            <Link
              to="/contact"
              className="no-underline inline-block font-semibold px-10 py-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 text-center"
              style={{ backgroundColor: builder.button, color: builder.textColor }}
            >
              {t('services.section.button')}
            </Link>
