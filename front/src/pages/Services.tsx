...
export default function Services() {
  ...
  const services: OurService[] = (t('services.ourServices', { returnObjects: true }) as any[]).map(service => ({
    ...service,
    icon: iconsMap[service.icon as string] || <FaLaptopCode />, // 👈 TypeScript ya entiende
  }));
  ...
  const inputBgStyle = useMemo(() => ({ backgroundColor: builder.inputBg }), [builder.inputBg]);
  ...
  return (
    <main className="overflow-x-hidden" style={cardStyle} id="services">
      {/* HERO */}
      ...
      <section className="py-16 px-4 text-center">
        <h2 className="text-3xl md:text-4xl font-bold mb-10" style={headingStyle}>
          {t('services.beneficts.title')}
        </h2>
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          {benefits.map(item => (
            <div key={item.title} className="p-6 rounded-xl shadow-lg hover:scale-105 transition-all duration-300" style={cardStyle}>
              <div className="text-4xl mb-4 text-blue-400 mx-auto">{item.icon}</div>
              <h3 className="text-xl font-semibold mb-2" style={headingStyle}>
                {item.title}
              </h3>
              <p>{item.text}</p>
            </div>
